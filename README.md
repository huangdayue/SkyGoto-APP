...
- **GOTO 进度弹窗**：GOTO 执行时实时显示目标/当前坐标，进度条

### 目录页面（CatalogScreen）
- **分类 Tab**：全部 / 梅西耶 / NGC / IC / 太阳系 / 恒星
- **搜索栏**：按名称/编号搜索
- **天体列表**：显示编号、名称、类型、坐标
- **GOTO 按钮**：点击后弹窗确认并执行 GOTO
- **太阳系特殊处理**：通过 Skyfield 实时计算行星位置（而非使用静态坐标）

### 设置页面（SettingsScreen）
- **限位设置**：东西方向限位角度、天顶限位
- **追踪设置**：追踪模式（恒星时/太阳/月球）
- **GOTO 设置**：GOTO 速率、到位容差
- **位置设置**：经度/纬度（用于太阳系计算）
- **设备信息**：固件版本、连接方式、关于
